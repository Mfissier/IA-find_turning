...
def tokenize_json_file(input_path, model_name, mod_dev = False):
    ...
    #
        ...
        #        
            try:
                return tokenizer.encode(data, truncation=True, padding='longest', max_length=512)
            except IndexError:
                return None
        #
        else:
    ...
    #
        if mod_dev :
            print_red(f"Folder processed_data not found")
        return None
    #
    output_path += "/" + get_filename_without_extension(input_path) + "_"+ get_filename_without_extension(model_name) +"processed_data.json" 
    try:
    # 
        with open(output_path, 'w', encoding='utf-8') as file:
